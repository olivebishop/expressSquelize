...
const User = db.User; // Assuming you have a User model for user information
...
const createUserRole = async (req, res) => {
  ...
  try {
    const response = await UserRole.create({ roleId, userId });
    if (!response) {
...
const deleteUserRoleById = async (req, res) => {
  ...
  try {
    const response = await UserRole.destroy({ where: { userId: usersId, roleId: rolesId } });
    if (response === 0) {
      res.status(404).json({ "message": "UserRole not found" });
    } else if (response) {
